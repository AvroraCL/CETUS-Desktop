...
桌面窗口只渲染状态并转发用户操作；DSH 生命周期、WebView2 会话策略、Windows 原生集成与配置持久化由各自模块拥有。WPF 仍是原生窗口宿主，WebView 消息桥只补充 CETUS 顶部工具栏和侧栏入口，不复制 DeepSeek Harness 的业务界面；右侧栏是覆盖主窗右缘的独立窗口，收放动画只改变侧栏窗口自身宽度。
