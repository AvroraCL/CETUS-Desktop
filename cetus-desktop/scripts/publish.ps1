...
<#
.SYNOPSIS
    Cetus packaging (M2 preview): self-contained publish + bundled, version-pinned
    node.exe and @deepseek-ai/dsh, then a portable zip.

    Output:
      dist\app\           runnable folder (double-click Cetus.Desktop.exe)
...
$root       = Split-Path -Parent $PSScriptRoot          # cetus-desktop
...
if (-not $iscc -or -not (Test-Path $iscc)) {
    $iscc = Join-Path $root "..\tools\innosetup\ISCC.exe"   # portable extract, e.g. F:\Cetus\tools
